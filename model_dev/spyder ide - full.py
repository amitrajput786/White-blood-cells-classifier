...
train_images = []
train_labels = [] 
for directory_path in glob.glob("PBC_dataset_normal_DIB_224/*"):
...
# test_images = []
# test_labels = [] 
# for directory_path in glob.glob("ds3/test/*"):
#     label = directory_path.split("\\")[-1]
#     print(label) 
#     for img_path in glob.glob(os.path.join(directory_path, "*.jpg")):
#         print(img_path)
...
#     shortcut = Conv2D(filters, (1, 1), strides=strides)(input)
...
#     x = Conv2D(filters, (1, 1), strides=1)(x)
#     x = BatchNormalization()(x)
#     x = Add()([x, shortcut])
#     x = ReLU()(x)
#     return x
...
# checkpoint_path = "results/ds3.keras" 
...
cs1 = tf.keras.layers.Conv2D(128,1,padding="same")(newfeature1)
cs1 = CAB_Block(cs1)
cs2 = tf.keras.layers.Conv2D(128,1,padding="same")(feature2)
cs2 = CAB_Block(cs2)
cs3 = tf.keras.layers.Conv2D(128,1,padding="same")(newfeature3)
cs3 = CAB_Block(cs3)
...
final_output = tf.keras.layers.Add()([AcsConv_layer1,
AcsConv_layer2,
AcsConv_layer3])
...
model = load_model('results/batch_size_32.keras', safe_mode=False)
